...
import procedure_create_1 as pro_1
import pandas  as pd
...
# source account details 
    ...
    list_Database=database_in_source['DATABASE_NAME'].tolist()
    tuple_Database=tuple(list_Database)
    ...
    connection_source= conn_sf.connected_sf(source_account_details)
    connection_source.execute("select distinct table_schema,table_catalog from SNOWFLAKE.account_usage.tables where deleted is null and table_schema not in('INFORMATION_SCHEMA','PUBLIC') and table_catalog not in('SNOWFLAKE_SAMPLE_DATA') and table_catalog in {tuple};".format(tuple=tuple_Database))
    list_values_source_Db = connection_source.fetchall()
    connection_source.close()
    ...
    ## procedure create block -----------------------------------------------------------------------------------------------------
    connection_source= conn_sf.connected_sf(source_account_details)
    connection_source.execute("select * from SNOWFLAKE.ACCOUNT_USAGE.PROCEDURES WHERE DELETED IS NULL and PROCEDURE_CATALOG in {set} and  procedure_definition is not null and length(procedure_definition) != 0;;".format(set=tuple_Database))
    list_procedure_source_avaiable = connection_source.fetchall()
    connection_source.close()
    ...
    #list of database available in source snowflake account 
    procedure_list_database=pd.DataFrame(list_procedure_source_avaiable,columns=column_names)
    print('----------------------------------------------list procedure avaiable ---------------------------------------')
    print(procedure_list_database)
    pro_1.procedure_list_source_from_main(procedure_list_database,source_account_details,target_account_details)
